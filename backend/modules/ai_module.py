                ...
                if "nvidia" in model.lower() or "mistral" in model.lower():
                    if GEMINI_API_KEY:
                        try:
                            ...
                            yield f"data: {json.dumps({'content': fallback_msg, 'chat_id': chat_id})}\\n\\n"
                            full_response += "\n\n*[Switching to Gemini fallback...]*\n\n"
                            ...
                            all_failed_msg = f'All AI providers failed. NVIDIA: {error_msg[:80]}. Gemini: {str(e2)[:80]}'
                            yield f"data: {json.dumps({'error': all_failed_msg})}\\n\\n"
                            return
